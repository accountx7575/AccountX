...
export function PurchaseOrderCreatePage() {
  ...
  const isInterState = useMemo(() => {
    if (!activeBusiness || !supplierId) return false;
    const s = suppliers?.find((x) => x.id === supplierId);
    return !!(s?.state && activeBusiness.state && s.state !== activeBusiness.state);
  }, [activeBusiness, suppliers, supplierId]);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast('Purchase order draft saved', 'success');
      navigate('/app/purchase-orders');
    },
    onError: (err: any) => toast(err.message || 'Failed to save purchase order', 'error'),
  });
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button variant="secondary" onClick={() => navigate('/app/purchase-orders')}><ArrowLeft className="h-4 w-4" /> Back</Button>}
      />
                ...
                <div className="grid grid-cols-[1fr_5rem_5rem_5rem_2rem] gap-2 items-center">
                  <select className="input text-xs" value={l.product_id ?? ''}
                    onChange={(e) => {
                      ...
                      if (p) updateLine(idx, { product_id: p.id, product_name: p.name, rate: p.purchase_price, tax_rate: p.tax_rate });
      ...
      <div className="sticky bottom-0 z-30 -mx-4 sm:-mx-6 px-4 sm:px-6 py-3 mt-5 border-t border-secondary-200/80 dark:border-zinc-800 bg-white/85 dark:bg-zinc-900/85 backdrop-blur-md">
        <div className="flex items-center justify-between gap-3">
          <Button variant="secondary" onClick={() => navigate('/app/purchase-orders')}>Cancel</Button>
